...
# 港股板块代表标的（每个板块用最贴近的代表股 / 指数，避免全部用 ^HSTECH 重复）
HK_SECTOR_TICKERS: dict[str, list[str]] = {
    "AI应用":  ["^HSTECH"],   # 恒生科技指数
    "芯片":    ["0981.HK"],   # 中芯国际
    "存储":    ["9999.HK", "0700.HK"],  # 网易、腾讯（港股无纯存储，用大科技代表）
    "机器人":  ["2382.HK", "1810.HK"],  # 舜宇光学、小米（智能硬件代表）
    "大消费":  ["9633.HK", "2331.HK"],  # 农夫山泉、李宁
    "石油":    ["0883.HK", "0386.HK"],  # 中海油、中石化
}
